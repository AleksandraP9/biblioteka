...
export class Biblioteka {
  ...
  crtajForme(host) {
      ...
      // else {
        fetch("https://localhost:5001/Centaar/UpisPolice/" + this.id, {
          ...
          if (p.ok) {
            ...
            this.dodajSelekt(polica.imePolice, polica.id); //Zove funkciju koja dodaje ime zanra u selekt
            location.reload();
          }
          else if (p.status == 406){
    ...
    dugmeZaIzmeniKnjigu.onclick = (ev) => {
      ...
      if (vrednostiKnjigeKojuMenjamo.imeKnjigeVrednost == "") {
       alert("Popunite polja za izmenu!");
        return;
      } 
      else {
        fetch(
          `https://localhost:5001/Centaar/IzmeniKnjigu/${vrednostiKnjigeKojuMenjamo.idPoliceVrednost}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  prebrojKnjigeUpolici(idPolice) {
    var element = document.querySelector(`.polica-id-${idPolice}`);
